...
export default function App() {
  ...
  // Optional initial values for contact form (used for actions like 'Post Job')
  const [contactInitial, setContactInitial] = useState<Record<string, string> | null>(null);
  ...
  // Handler invoked when user wants to post a job — prefill contact form
  const handlePostJobClick = () => {
    setContactInitial({
      subject: 'Job Posting Request',
      message: 'I would like to post a new job.\n\nTitle:\nLocation:\nType (Full-time/Contract):\nSkills Required:\nCompensation Range:\nContact Person & Email:'
    });
    scrollToSection('contact');
    // clear initial after short delay to avoid persisting across unrelated opens
    setTimeout(() => setContactInitial(null), 1200);
  };
  ...
  return (
    <div className="min-h-screen relative bg-slate-50/20 text-slate-900 font-sans selection:bg-blue-100 selection:text-[#2563EB]">
      ...
      <Navbar onContactClick={() => scrollToSection('contact')} onPostJobClick={handlePostJobClick} />
      ...
      <ContactSection initialFormData={contactInitial} />
